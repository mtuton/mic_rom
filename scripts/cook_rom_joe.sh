...
include_app_in_rom()
{
	case $1 in
		...
		BuddiesNow)				include_app=1 ;; # Buddies Now - widget for touchwiz launcher
		Days)					include_app=1 ;; # Days - widget for touchwiz launcher
		Dlna)					include_app=1 ;; # Samsung All Share - wireless communication with other 
									 #   Samsung dlna compatible devices
		DualClock)				include_app=1 ;; # Dual Clock - have two clocks on your homescreen for different timezones
		Email)					include_app=1 ;; # this will be replaced by a custom Email app
		GoogleFeedback)				include_app=1 ;; # Google Feedback
		...
		MiniDiary)				include_app=1 ;; # Mini Diary
		...
		PressReader)				include_app=1 ;; # Press Reader
		...
		WriteandGo)				include_app=1 ;; # Write and Go
